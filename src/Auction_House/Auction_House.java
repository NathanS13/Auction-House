...
public class Auction_House extends Thread {
    ...
    /**The method sendBid uses the agent's secret key to make a bid on the item passed in with the chosen bid amount.
     * It first finds the item and sets the timer if it has not been initialized. Otherwise, it updates the current
     * ticking time. It checks that the bidder has .01 over the current bid and locks in the bidder's secret key
